...
def evaluate(prefix="./", mode="dev", evaluate_dir=None, dataset_dir=None, evaluate_prefix=None,
             output_file=None, output_dir=None, python_evaluate=False,
             triplet=False):
    '''
    dataset_dir: separate evaluation script dir and dataset dir
    '''
    sclite_path = "./software/sclite"
    print(os.getcwd())
    os.system(f"bash {evaluate_dir}/preprocess.sh {prefix + output_file} {prefix}tmp.ctm {prefix}tmp2.ctm")
    os.system(f"cat {dataset_dir}/{evaluate_prefix}-{mode}.stm | sort  -k1,1 > {prefix}tmp.stm")
    ...
    if python_evaluate:
        ret = wer_calculation(f"{dataset_dir}/{evaluate_prefix}-{mode}.stm", f"{prefix}out.{output_file}")
        if triplet:
            wer_calculation(
                f"{dataset_dir}/{evaluate_prefix}-{mode}.stm",
